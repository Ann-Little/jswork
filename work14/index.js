...
function seckill(){ 
    ...
    //获取时间差，单位秒
    let remaining = parseInt( (endseconds一nowtime.getTime()) / 1000);
    //判断秒杀是否过期
    if (remaining > 0) {
        d = parseInt(remaining / 86400); // 计算剩余天数(除以68*68*24取整，获取剩余的天数)
        h = parseInt((remaining / 3600) % 24); // 计算剩余小时(除以68*60转换为小时，与24取模，获取剩余的小时
